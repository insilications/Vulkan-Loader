...
unset no_proxy
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1597234683
mkdir -p clr-build
pushd clr-build
...
%cmake .. -DBUILD_TESTS=1 -DBUILD_STATIC_LIBS:BOOL=ON -DBUILD_STATIC_LIBS=1 -DENABLE_SHARED:bool=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS:bool=ON
make  %{?_smp_mflags}
...
%cmake -DLIB_INSTALL_DIR:PATH=/usr/lib32 -DCMAKE_INSTALL_LIBDIR=/usr/lib32 -DLIB_SUFFIX=32 .. -DBUILD_TESTS=1 -DBUILD_STATIC_LIBS:BOOL=ON -DBUILD_STATIC_LIBS=1 -DENABLE_SHARED:bool=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS:bool=ON
make  %{?_smp_mflags}
...
/usr/lib64/libvulkan.so.1.2.149

%files lib32
%defattr(-,root,root,-)
/usr/lib32/libvulkan.so.1
/usr/lib32/libvulkan.so.1.2.149
